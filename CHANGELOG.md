...
- **Stale cross-provider session models now normalize correctly for non-standard providers** — `_normalize_provider_id()` previously returned `""` for any provider not in its short prefix list (ollama-cloud, deepseek, xai, etc.), causing `_resolve_compatible_session_model()` to early-return "no active provider" and pass stale models through unchanged. The resolver now also keeps the raw lowercased active-provider slug as a fallback comparator, so a session model on a known namespace (e.g. `openai/gpt-5.4-mini`) is still detected as stale relative to an unlisted active provider (e.g. `ollama-cloud`) and replaced with the configured default. (`api/routes.py`) Closes #1023.
